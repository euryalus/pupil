...
def calibrate_3d_binocular(g_pool, matched_binocular_data, pupil0, pupil1):
    ...
    ref_points, gaze0_dir, gaze1_dir = calibrate.preprocess_3d_data(matched_binocular_data, g_pool)

    if len(ref_points) < 1 or len(gaze0_dir) < 1 or len(gaze1_dir) < 1:
    ...
    ref_points = np.array(ref_points).reshape(-1, 3)
    magnitude = np.linalg.norm(ref_points, axis=1).reshape((-1, 1))
    ref_dir = ref_points / magnitude

    gaze0_dir = np.array(gaze0_dir)
    initial_R0, initial_t0 = find_rigid_transform(gaze0_dir*magnitude, ref_points)
    ...
    gaze1_dir = np.array(gaze1_dir)
    initial_R1, initial_t1 = find_rigid_transform(gaze1_dir*magnitude, ref_points)
    ...
    eye0 = {"observations": gaze0_dir,
            "translation": initial_t0,
            "rotation": initial_rotation0,
            'fix': []} #TODO ???

    eye1 = {"observations": gaze1_dir,
            "translation": initial_t1,
...
def calibrate_3d_monocular(g_pool, matched_monocular_data):
    ...
    # hardcoded_translation0 = np.array([20, 15, -20]) # TODO Delete
    # hardcoded_translation1 = np.array([-40, 15, -20])
    # TODO model the world as cv2 pinhole camera with distorion and focal in ceres.
    # right now we solve using a few permutations of K
    ...
    ref_points, gaze_dir, _ = calibrate.preprocess_3d_data(matched_monocular_data, g_pool)
    # save_object((ref_points,gaze_dir),os.path.join(g_pool.user_dir, "testdata"))
    if len(ref_points) < 1 or len(gaze_dir) < 1:
    ...
    gaze_dir = np.array(gaze_dir)
    initial_R, initial_t = find_rigid_transform(gaze_dir*magnitude, ref_points)
    ...
    t_world = np.array(eye['translation'])
    R_world = math_helper.quaternion_rotation_matrix(np.array(eye['rotation']))

    # # inverse is pose of eye in world coords# TODO Delete
    # R_eye = R_world.T
    # t_eye = np.dot(R_eye, -t_world)

    def toWorld(p):
        return np.dot(R_world, p)+t_world

    # points_in_world = [toWorld(p).tolist() for p in points_in_eye]

    points_a = []  # world coords
    points_b = []  # cam2 coords
    for a, b, point in zip(world['observations'], eye['observations'], points):

        line_a = np.array([0, 0, 0]), np.array(a)  # observation as line
        line_b = toWorld(np.array([0, 0, 0])), toWorld(b)  # cam2 observation line in world coords
        close_point_a, _ = math_helper.nearest_linepoint_to_point(point, line_a)
        close_point_b, _ = math_helper.nearest_linepoint_to_point(point, line_b)
    ...
    sphere_translation_world = np.dot(R_world, sphere_translation)
    camera_translation = t_world - sphere_translation_world
    eye_camera_to_world_matrix = np.eye(4)
    eye_camera_to_world_matrix[:3, :3] = R_world
